...
const CodingQuestions = ({ testId }) => {
  ...
  const fetchCodingQuestions = async () => {
    setLoading(true);
    try {
      // Get test to fetch its problems
      const testRes = await axios.get(`${API_BASE}/contest/${testId}`);
      if (testRes.status === 200 && testRes.data.contest) {
        const problems = testRes.data.contest.problems || [];
  ...
  const fetchDefaultMarkdown = async () => {
    // Just set default template - no API call needed
                  ...
                  {codingQuestions.map((q) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-700 max-h-48 overflow-y-auto prose prose-sm">
                        ...
                        >
                          {renderQuestionPreview(q.statement || q.markdownContent || '')}
                        </ReactMarkdown>
                      </div>
                    </div>
                  ))}
