...
export default function UpdateTestPage({ testId, title, description, testTime, totalTime }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (TestTime) {
      payload.startTime = new Date(TestTime).toISOString();
    }

    if (TotalTime) {
      const totalTimeInt = parseInt(TotalTime);
      if (!isNaN(totalTimeInt) && totalTimeInt > 0) {
        const endTime = new Date(new Date(TestTime).getTime() + totalTimeInt * 60000);
    ...
    try {
      ...
      const res = await axios.put(
        `${API_BASE}/contest/update/${testId}`, 
        payload, 
        { headers: { token } }
      );
      ...
      console.error(err);
      setError(err.response?.data?.error || "Error updating test");
    } finally {
      setIsSubmitting(false);
    }
  };
